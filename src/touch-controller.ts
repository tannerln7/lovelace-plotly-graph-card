...
export class TouchController {
  ...
  private singleFingerResetEligible = false;
  ...
  private startSingleFingerZoom(e: TouchEvent) {
    ...
    this.singleFingerResetEligible = this.isMainDraggerEvent(e);
    this.lastTouches = e.touches;
  ...
  onTouchStart = async (e: TouchEvent) => {
    ...
    if (this.isEnabled && e.touches.length === 1
      ...
      && (touchMode === "plotly" || this.isMainDraggerEvent(e))) {
      this.startSingleFingerZoom(e);
      return;
    }
    if (e.touches.length === 1 && this.touchTooltipArmed
      && this.isMainDraggerEvent(e) && this.isHoverLabelTouch(e.touches[0])) {
  ...
  async handleSingleFingerZoom(e: TouchEvent) {
    ...
    e.stopImmediatePropagation();
    const touch = e.touches[0];
    if (this.singleFingerStart && Math.max(
      Math.abs(touch.clientX - this.singleFingerStart.x),
      Math.abs(touch.clientY - this.singleFingerStart.y),
    ) >= MIN_TOUCH_DRAG_PX) this.singleFingerDragged = true;
  ...
  onTouchEnd = (e: TouchEvent) => {
    ...
    if (this.state !== "idle") {
      if (this.state === "one finger" && !this.singleFingerDragged
        ...
        && this.singleFingerResetEligible)
        this.dispatchPlotlyTap(e.changedTouches[0]);
      this.onZoomEnd();
      this.state = "idle";
      this.singleFingerStart = undefined;
      this.singleFingerDragged = false;
      this.singleFingerResetEligible = false;
    }
  };

  onTouchCancel = (e: TouchEvent) => {
    if (this.continueHover(e)) return;
    if (this.state === "one finger") {
      this.onZoomEnd();
      this.state = "idle";
      this.singleFingerStart = undefined;
      this.singleFingerDragged = false;
