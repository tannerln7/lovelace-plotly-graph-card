...
While Touch Hover is selected, a stationary plot touch retains Plotly click behavior and dragging scrubs without clicking. A persistent tooltip created by touch can be dismissed by tapping it in either Touch Hover or native touch mode; axes and other controls remain native. One-finger double-tap-drag zoom is replaced by Hover in this mode; two-finger custom pinch remains available unless `disable_pinch_to_zoom` is set. Disabling pinch does not disable Touch Hover.
