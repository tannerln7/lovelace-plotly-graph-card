...
While Touch Hover is selected, a stationary plot touch retains Plotly click behavior and single-finger dragging scrubs without clicking. A persistent tooltip created by touch can be dismissed by tapping it in either Touch Hover or native touch mode; axes and other controls remain native. The existing double-tap-drag zoom and two-finger custom pinch remain available unless `disable_pinch_to_zoom` is set. Disabling pinch does not disable Touch Hover.
